...
router.put('/:id', authenticate, async (req, res) => {
  try {
    ...
    const { title, type, priority, description, assignee_id, selected_options } = req.body
    await execute(
    ...
    if (selected_options && Array.isArray(selected_options)) {
      await execute('DELETE FROM ticket_selected_options WHERE ticket_id = ?', [req.params.id])
      for (const opt of selected_options) {
        await execute('INSERT INTO ticket_selected_options (id, ticket_id, option_id, option_label) VALUES (?, ?, ?, ?)',
          [uuidv4(), req.params.id, opt.id, opt.label])
      }
    }
    const ticket = await getTicketWithDetails(req.params.id)
    res.json({ ticket })
