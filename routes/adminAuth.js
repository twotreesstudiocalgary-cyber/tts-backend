...
router.get('/me', authenticate, async (req, res) => {
  try {
    const [rows] = await execute('SELECT id, name, email, role, status FROM users WHERE id = ?', [req.user.id])
...
router.post('/team/invite', authenticate, requireSuperAdmin, async (req, res) => {
  try {
    ...
    const [existing] = await execute('SELECT id FROM users WHERE email = ?', [email])
    ...
    await execute('INSERT INTO users (id, name, email, password, role) VALUES (?, ?, ?, ?, ?)', [id, name, email, hashed, 'staff'])
...
router.put('/team/:id/toggle-status', authenticate, requireSuperAdmin, async (req, res) => {
  try {
    const [rows] = await execute('SELECT * FROM users WHERE id = ?', [req.params.id])
    if (rows.length === 0) return res.status(404).json({ message: 'User not found' })
    if (rows[0].role === 'superadmin') return res.status(403).json({ message: 'Cannot deactivate super admin' })

    const newStatus = rows[0].status === 'active' ? 'inactive' : 'active'
    await execute('UPDATE users SET status = ? WHERE id = ?', [newStatus, req.params.id])
...
router.put('/change-password', authenticate, async (req, res) => {
  try {
    ...
    const [rows] = await execute('SELECT password FROM users WHERE id = ?', [req.user.id])
