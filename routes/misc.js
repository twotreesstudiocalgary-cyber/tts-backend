...
router.post('/ticket-types', authenticate, requireSuperAdmin, async (req, res) => {
  try {
    ...
    const [existing] = await pool.execute('SELECT id FROM ticket_types WHERE name = ?', [name.trim()])
    if (existing.length > 0) return res.status(409).json({ message: 'This type already exists' })
    const id = uuidv4()
    await pool.execute('INSERT INTO ticket_types (id, name) VALUES (?, ?)', [id, name.trim()])
...
router.delete('/ticket-types/:id', authenticate, requireSuperAdmin, async (req, res) => {
  try {
    await pool.execute('UPDATE ticket_types SET active = 0 WHERE id = ?', [req.params.id])
...
router.put('/clients/:id/toggle-status', authenticate, requireSuperAdmin, async (req, res) => {
  try {
    ...
    await pool.execute('UPDATE clients SET status = ? WHERE id = ?', [newStatus, req.params.id])
...
router.get('/reports/summary', authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    const [[{ open }]] = await pool.execute("SELECT COUNT(*) as open FROM tickets WHERE status = 'new'")
    const [[{ inprogress }]] = await pool.execute("SELECT COUNT(*) as inprogress FROM tickets WHERE status = 'inprogress'")
    const [[{ complete }]] = await pool.execute("SELECT COUNT(*) as complete FROM tickets WHERE status = 'complete'")
    const [[{ overdue }]] = await pool.execute("SELECT COUNT(*) as overdue FROM tickets WHERE status IN ('new','inprogress') AND updated_at < DATE_SUB(NOW(), INTERVAL 3 DAY)")
    const [[{ unpaidInvoices }]] = await pool.execute("SELECT COUNT(*) as unpaidInvoices FROM invoices WHERE status = 'unpaid'")
    const [[{ unpaidAmount }]] = await pool.execute("SELECT COALESCE(SUM(amount),0) as unpaidAmount FROM invoices WHERE status = 'unpaid'")
...
router.post('/invoices/:id/confirm-payment', authenticate, async (req, res) => {
  try {
    await pool.execute("UPDATE invoices SET status = 'paid', paid_at = NOW() WHERE id = ?", [req.params.id])
    res.json({ success: true })
  } catch (err) {
    res.status(500).json({ message: 'Server error' })
  }
